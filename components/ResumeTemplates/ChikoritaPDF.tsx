...
function makeStyles(font: TBFont, accent: string, stylePreset = getResumeStylePreset(), pageWidth = getResumePageWidth()) {
  ...
  return StyleSheet.create({
    ...
    contactLine: {
      fontSize: stylePreset.metaFont, color: "#555555", flexDirection: "row",
      flexWrap: "wrap", justifyContent: "center", gap: 12,
    },
    sectionTitle: {
      fontSize: stylePreset.sectionFont, fontFamily: bold, textTransform: "uppercase",
      letterSpacing: stylePreset.letterSpacing, color: accent, marginBottom: 5,
      paddingBottom: 2, borderBottomWidth: 0.5, borderBottomColor: accent,
    },
...
export function ChikoritaPDF({ data }: Props) {
            ...
            {educations.filter((e) => e.school || e.degree).map((e) => {
              ...
              return (
                <View key={e.id} style={{ marginBottom: Math.max(5, preset.entryGap - 2) }}>
                  ...
                  {e.coursework ? <Text style={styles.metaSmall}>Coursework: {e.coursework}</Text> : null}
                </View>
              );
            })}
          </View>
            ...
            {projects.filter((p) => p.name).map((p) => {
              ...
              return (
                <View key={p.id} style={{ marginBottom: Math.max(5, preset.entryGap - 2) }}>
                  <View style={styles.jobRow}>
                    <Text style={styles.jobTitle}>{p.name}{p.tech ? `  |  ${p.tech}` : ""}</Text>
              ...
              <View style={{ flexDirection: "row", marginBottom: 5 }}>
                {[0, 1, 2].map((col) => (
                  <View key={col} style={{ width: "33.33%" }}>
                    {[featuredWithSkill[col], featuredWithSkill[col + 3]].filter(Boolean).map((fs, i) => (
      ...
      <Page size="LETTER" style={styles.page}>
        <View style={styles.headerBox}>
          ...
          <View style={styles.contactLine}>
            {contactParts.map((c, i) => <Text key={i}>{c}</Text>)}
          </View>
        </View>
