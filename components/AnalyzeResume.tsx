    ...
    try {
      ...
      if (!resp.ok) {
        if (resp.status === 429 && json?.code === "daily_scan_limit_reached") {
          ...
          const freeLimit = Number.isFinite(limit) && limit > 0 ? limit : FREE_DAILY_SCAN_LIMIT;
          setFeedbackToast(
            json?.reason === "anonymous_daily_ip_limit"
              ? `Free scans used for today — sign in (it's free) for ${freeLimit} scans/day and saved reports.`
