...
import type { RatingsData, DetailedRatingItem, DetailedCategory, JobTitleRating, KeywordsRating } from "@/lib/types";
...
function KeywordsSection({ keywords }: { keywords: KeywordsRating }) {
  ...
  return (
    <div>
      <div style={{ display: "flex", gap: 12, marginBottom: 20 }}>
        <div style={{ flex: 1, padding: "16px 20px", borderRadius: 12, background: "rgba(52,211,153,0.06)", border: "1px solid rgba(52,211,153,0.2)", textAlign: "center" }}>
          <div style={{ fontSize: 28, fontWeight: 800, color: "var(--green, #34d399)", letterSpacing: -1 }}>{found.length}</div>
          <div style={{ fontSize: 11, fontWeight: 700, color: "var(--green, #34d399)", letterSpacing: 0.4, textTransform: "uppercase", marginTop: 2 }}>Found</div>
        </div>
        <div style={{ flex: 1, padding: "16px 20px", borderRadius: 12, background: "rgba(248,113,113,0.06)", border: "1px solid rgba(248,113,113,0.2)", textAlign: "center" }}>
          <div style={{ fontSize: 28, fontWeight: 800, color: "#f87171", letterSpacing: -1 }}>{missing.length}</div>
          <div style={{ fontSize: 11, fontWeight: 700, color: "#f87171", letterSpacing: 0.4, textTransform: "uppercase", marginTop: 2 }}>Missing</div>
        </div>
      </div>
      {missing.length > 0 && (
        <div style={{ marginBottom: 16 }}>
          <div style={{ fontSize: 11, fontWeight: 700, color: "#f87171", letterSpacing: 0.3, textTransform: "uppercase", marginBottom: 8 }}>Missing Keywords</div>
          <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
            {missing.map((kw, i) => (
              <span key={i} style={{ padding: "4px 10px", borderRadius: 6, background: "rgba(248,113,113,0.08)", border: "1px solid rgba(248,113,113,0.2)", fontSize: 12, color: "#f87171", fontWeight: 500 }}>
                {kw}
              </span>
            ))}
          </div>
      ...
      {found.length > 0 && (
        <div>
          <div style={{ fontSize: 11, fontWeight: 700, color: "var(--green, #34d399)", letterSpacing: 0.3, textTransform: "uppercase", marginBottom: 8 }}>Found Keywords</div>
          <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
            {found.map((kw, i) => (
              <span key={i} style={{ padding: "4px 10px", borderRadius: 6, background: "rgba(52,211,153,0.08)", border: "1px solid rgba(52,211,153,0.2)", fontSize: 12, color: "var(--green, #34d399)", fontWeight: 500 }}>
                {kw}
              </span>
            ))}
          </div>
...
export default function DetailedRatingsView({
  ...
  const tabs: { id: Tab; label: string; score: string; color: string }[] = [
    ...
    },
    {
      id: "keywords",
      label: "Keywords",
      score: `${keywords.found_count} Found`,
      color: scoreColor(keywords.found_count > 0 ? Math.round((keywords.found_count / Math.max(keywords.total_count, 1)) * 100) : 0),
    },
  ];
